...
export default function PerformanceManagement() {
  ...
  useEffect(() => {
    const fetchPips = async () => {
      setPipLoading(true);
      try {
        const res = await fetch(APIURL +'/api/performance-improvement-plans');
        if (!res.ok) throw new Error('Failed to fetch PIPs');
        const data = await res.json();
        // Map API data to PerformanceImprovementPlan[]
        const mapped = data.map((item: PerformanceImprovementPlan) => ({
          id: item.id,
  ...
  const handlePipSubmit = async () => {
    try {
      ...
      // Convert date string to array [YYYY, M, D]
      const dateToArr = (d: string) => {
      ...
      const pipBody = {
        ...
        startDate: dateToArr(pipFormData.startDate),
        endDate: dateToArr(pipFormData.endDate),
        objectives: pipFormData.objectives,
        actions: pipFormData.actions,
        support: pipFormData.support,
        reviewDate: dateToArr(pipFormData.reviewDate),
  ...
  const handlePipDelete = async (id: number | undefined) => {
    ...
    if (window.confirm('Are you sure you want to delete this Performance Improvement Plan?')) {
      try {
        const res = await fetch(APIURL +`/api/performance-improvement-plans/${id}`, {
          method: 'DELETE',
        });
